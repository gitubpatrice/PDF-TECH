...
class _CreatePdfScreenState extends State<CreatePdfScreen> {
  ...
  Future<void> _addImage() async {
    ...
    setState(() {
      _blocks.add(
        _Block(type: _BlockType.image, imagePath: res.files.single.path),
      );
    });
  }
    ...
    try {
      ...
      for (final b in _blocks) {
        if (b.type == _BlockType.image && b.imagePath != null) {
          try {
            ...
            page.graphics.drawImage(
              bitmap,
              Rect.fromLTWH(_kPdfMargin, cursorY, dw, dh),
            );
            cursorY += dh + 12;
          } catch (_) {
        ...
        final brush = PdfSolidBrush(
          PdfColor(b.color.r.toInt(), b.color.g.toInt(), b.color.b.toInt()),
        );
